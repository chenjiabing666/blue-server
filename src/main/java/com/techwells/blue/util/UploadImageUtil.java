...
public class UploadImageUtil {
	 ...
	 */
 	 ...
 	 * @param width      缩略图的宽度
 	 * @param height     缩略图的高度
 	 * @param uploadUrl   服务器上的url，即是能够网上访问的链接
 	 * @param realUploadPath   存储在服务器地址，即是缩略图片在服务器的本地存储的位置   
 	 * @return  返回网上访问的真实路径
 	 */
 	public static String thumbnailUploadImage(String fileName,InputStream inputStream,int width,
			int height, String uploadUrl, String realUploadPath) {
		String des = realUploadPath + "thum/"; // 在服务器实际存储的文件夹
		File targetFile = new File(des, fileName);
		// 如果文件夹不存在，那么就创建
